...
export class Renderer2D {
  ...
  render(model) {
    ...
    for (const wp of wallPolygons) { this._drawWallPolygon(wp); }
    for (const wall of storey.walls) {
      for (const opening of wall.openings) { this._drawOpening(wall, opening); }
    }
  ...
  _drawOpening(wall, opening) {
    ...
    if (opening.type === 'door') { this._drawDoor(p1, p2, nx, ny, halfTpx, opening); }
    else { this._drawWindow(p1, p2, nx, ny, halfTpx, opening); }
  }
  ...
  for (let i = 0; i < walls.length; i++) {
    for (let j = i + 1; j < walls.length; j++) {
      ...
      for (const { p1, p2, e1, e2 } of pairs) {
        ...
        const leftMiter = lineIntersect(d1.leftStart, d1.leftEnd, d2.leftStart, d2.leftEnd);
        const rightMiter = lineIntersect(d1.rightStart, d1.rightEnd, d2.rightStart, d2.rightEnd);
        ...
        if (e1 === 'end') { d1.endLeft = leftMiter; d1.endRight = rightMiter; }
        else { d1.startLeft = leftMiter; d1.startRight = rightMiter; }
        if (e2 === 'end') { d2.endLeft = leftMiter; d2.endRight = rightMiter; }
        else { d2.startLeft = leftMiter; d2.startRight = rightMiter; }
      }
    }
  }
