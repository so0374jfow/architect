...
export class Renderer3D {
  constructor(container) {
    ...
    this.wallMaterial = new THREE.MeshStandardMaterial({ color: 0xe0e0e0, roughness: 0.7, metalness: 0.0 });
    this.doorMaterial = new THREE.MeshStandardMaterial({ color: 0x4fc3f7, roughness: 0.3, metalness: 0.1, transparent: true, opacity: 0.6 });
    this.windowMaterial = new THREE.MeshStandardMaterial({ color: 0x81c784, roughness: 0.1, metalness: 0.2, transparent: true, opacity: 0.4 });
    this.floorMaterial = new THREE.MeshStandardMaterial({ color: 0x3a3a4e, roughness: 0.8 });
  ...
  rebuild(model) {
    ...
    for (const storey of model.storeys) { this._buildStorey(storey); }
  }
  ...
  _buildStorey(storey) {
    ...
    if (storey.walls.length > 0) {
      ...
      floor.position.set((minX + maxX) / 2, elevation - 0.075, (minZ + maxZ) / 2);
  ...
  _buildWallWithOpenings(wall, elevation, wallLen, angle, startExt = 0, endExt = 0) {
    ...
    for (const opening of openings) {
      ...
      segments.push({ type: opening.type, start: openStart, end: openEnd, height: opening.height, bottomY: opening.sillHeight });
      cursor = openEnd;
    }
